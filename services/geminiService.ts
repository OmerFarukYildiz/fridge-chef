...
export const getGeminiModel = (): string => {
  return process.env.EXPO_PUBLIC_GEMINI_MODEL || process.env.GEMINI_MODEL || 'gemini-2.0-flash';
};
...
## KRİTİK KURAL 4 — Dil: ${getLanguageName(userProfile?.language)}${prefsText}${diversityText}${excludeText}`;
  ...
  let response: any;
  try {
    console.log('[GeminiService] generateContent options request:', {
      model,
      base64Length: base64Image.length,
      excludeCount: excludeRecipes.length,
    ...
    console.error('[GeminiService] generateContent options failed:', {
      model,
      base64Length: base64Image.length,
      excludeCount: excludeRecipes.length,
...
## KRİTİK KURAL 4 — Dil: ${getLanguageName(userProfile?.language)}${prefsText}`;
  ...
  let response: any;
  try {
    console.log('[GeminiService] generateContent detail request:', {
      model,
      recipeName: selectedOption.tarifAdi,
    ...
    console.error('[GeminiService] generateContent detail failed:', {
      model,
      recipeName: selectedOption.tarifAdi,
      base64Length: base64Image?.length ?? 0,
      message: getUnknownErrorMessage(err),
...
## KRİTİK KURAL 3 — Dil: ${getLanguageName(userProfile?.language)}${prefsText}${diversityText}`;
  ...
  const userPrompt = `Kilerimde şu malzemeler bozulmak üzere: ${expiringItems.length > 0 ? expiringItems.join(', ') : 'belirtilmedi'
    }. Tüm kiler içeriğim: ${allPantryItems.length > 0 ? allPantryItems.join(', ') : 'belirtilmedi'
